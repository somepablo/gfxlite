...
export class GLTFLoader {
    ...
    private async loadTextures() {
        ...
        const imagePromises = (this.json.images || []).map(async (img, index) => {
            ...
            if (img.uri) {
                ...
                source = await createImageBitmap(blob);
                 ...
                 if (!buffer) throw new Error(`Buffer ${bufferView.buffer} not found for image ${index}`);
                 
                 const blob = new Blob([new Uint8Array(buffer, bufferView.byteOffset || 0, bufferView.byteLength)], { type: img.mimeType });
                 source = await createImageBitmap(blob);
            } else {
        ...
        loadedImages.forEach(({ index, source }) => imageMap.set(index, source));
        ...
        this.json.textures.forEach((tex, index) => {
            ...
            const sampler = tex.sampler !== undefined && this.json!.samplers ? this.json!.samplers[tex.sampler] : {};
    ...
    private convertWrapMode(mode?: number): GPUAddressMode {
        switch (mode) {
            case GLTFConstants.CLAMP_TO_EDGE: return "clamp-to-edge";
            case GLTFConstants.MIRRORED_REPEAT: return "mirror-repeat";
            case GLTFConstants.REPEAT: return "repeat";
    ...
    private convertMagFilter(filter?: number): GPUFilterMode {
         switch (filter) {
            case GLTFConstants.NEAREST: return "nearest";
            case GLTFConstants.LINEAR: return "linear";
            default: return "linear";
        }
    }
    ...
    private loadMaterials() {
        ...
        this.json.materials.forEach((mat, index) => {
            ...
            if (mat.pbrMetallicRoughness?.baseColorTexture) {
                material.baseColorMap = this.textures.get(mat.pbrMetallicRoughness.baseColorTexture.index) || null;
            }
            if (mat.pbrMetallicRoughness?.metallicRoughnessTexture) {
                material.metallicRoughnessMap = this.textures.get(mat.pbrMetallicRoughness.metallicRoughnessTexture.index) || null;
            }
            ...
            if (mat.emissiveTexture) {
                material.emissiveMap = this.textures.get(mat.emissiveTexture.index) || null;
            }
            if (mat.occlusionTexture) {
                material.aoMap = this.textures.get(mat.occlusionTexture.index) || null;
                material.uniforms.occlusionStrength = mat.occlusionTexture.strength ?? 1.0;
            }
    ...
    private loadMeshes() {
        ...
        this.json.meshes.forEach((meshDef, index) => {
            ...
            meshDef.primitives.forEach((prim) => {
                ...
                const positions = this.getAccessorData(positionAccessor) as Float32Array;
                ...
                const geometry = new Geometry(positions, indices, normals, uvs, tangents);
    ...
    private getAccessorData(accessor: GLTFAccessor): TypedArray {
        ...
        if (stride === elementSize) {
             // Tightly packed
             return this.createTypedArray(buffer, byteOffset, totalElements, componentType);
        } else {
                 ...
                 for (let c = 0; c < numComponents; c++) {
                     ...
                     switch (componentType) {
                        case GLTFConstants.BYTE: 
                            outputView.setInt8(outputOffset + offsetVal, inputView.getInt8(inputOffset + offsetVal)); break;
                        case GLTFConstants.UNSIGNED_BYTE: 
                            outputView.setUint8(outputOffset + offsetVal, inputView.getUint8(inputOffset + offsetVal)); break;
                        case GLTFConstants.SHORT: 
                            outputView.setInt16(outputOffset + offsetVal, inputView.getInt16(inputOffset + offsetVal, true)); break;
                        case GLTFConstants.UNSIGNED_SHORT: 
                            outputView.setUint16(outputOffset + offsetVal, inputView.getUint16(inputOffset + offsetVal, true)); break;
                        case GLTFConstants.UNSIGNED_INT: 
                            outputView.setUint32(outputOffset + offsetVal, inputView.getUint32(inputOffset + offsetVal, true)); break;
                        case GLTFConstants.FLOAT: 
                            outputView.setFloat32(outputOffset + offsetVal, inputView.getFloat32(inputOffset + offsetVal, true)); break;
                     }
                 }
             }
             
             return this.createTypedArray(outputBuffer, 0, totalElements, componentType);
        }
    }
    ...
    private getComponentSize(componentType: number): number {
         switch (componentType) {
            case GLTFConstants.BYTE: return 1;
            case GLTFConstants.UNSIGNED_BYTE: return 1;
            case GLTFConstants.SHORT: return 2;
            case GLTFConstants.UNSIGNED_SHORT: return 2;
            case GLTFConstants.UNSIGNED_INT: return 4;
            case GLTFConstants.FLOAT: return 4;
            default: return 0;
        }
    }

    private createTypedArray(buffer: ArrayBuffer, byteOffset: number, length: number, componentType: number): TypedArray {
        switch (componentType) {
            case GLTFConstants.BYTE: return new Int8Array(buffer, byteOffset, length);
            case GLTFConstants.UNSIGNED_BYTE: return new Uint8Array(buffer, byteOffset, length);
            case GLTFConstants.SHORT: return new Int16Array(buffer, byteOffset, length);
            case GLTFConstants.UNSIGNED_SHORT: return new Uint16Array(buffer, byteOffset, length);
            case GLTFConstants.UNSIGNED_INT: return new Uint32Array(buffer, byteOffset, length);
            case GLTFConstants.FLOAT: return new Float32Array(buffer, byteOffset, length);
    ...
    private buildScene(): Object3D { 
        ...
        const sceneDef = this.json?.scenes ? this.json.scenes[sceneIndex] : undefined;
    ...
    private parseNode(index: number): Object3D {
        ...
        if (nodeDef.matrix) {
            object.localMatrix.set(
                nodeDef.matrix[0], nodeDef.matrix[1], nodeDef.matrix[2], nodeDef.matrix[3],
                nodeDef.matrix[4], nodeDef.matrix[5], nodeDef.matrix[6], nodeDef.matrix[7],
                nodeDef.matrix[8], nodeDef.matrix[9], nodeDef.matrix[10], nodeDef.matrix[11],
                nodeDef.matrix[12], nodeDef.matrix[13], nodeDef.matrix[14], nodeDef.matrix[15]
            );
            object.localMatrix.decompose(object.position, object.rotation, object.scale);
        } else {
            if (nodeDef.translation) {
                object.position.set(nodeDef.translation[0], nodeDef.translation[1], nodeDef.translation[2]);
            }
            if (nodeDef.rotation) {
                object.rotation.set(nodeDef.rotation[0], nodeDef.rotation[1], nodeDef.rotation[2], nodeDef.rotation[3]);
            }
            if (nodeDef.scale) {
    ...
    private createMeshNode(geometries: Geometry[], meshDef: GLTFMesh, originalObject: Object3D): Object3D {
         if (geometries.length === 1) {
             const primitiveDef = meshDef.primitives[0];
             const materialIndex = primitiveDef.material;
             let material = this.materials.get(materialIndex!);
             ...
             originalObject.children.forEach((c: Object3D) => mesh.add(c));
