...
export class MainRenderPhase extends RenderPhase {
    ...
    private clearColor: { r: number; g: number; b: number; a: number } = { r: 0.1, g: 0.1, b: 0.1, a: 1.0 };
    ...
    private initBindGroupLayouts(): void {
        ...
        this.textureBindGroupLayout = this.device.createBindGroupLayout({
            ...
            entries: [
                ...
                { binding: 6, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: "float", viewDimension: "cube" } }, // irradianceMap
                { binding: 7, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: "float", viewDimension: "cube" } }, // prefilteredMap
                { binding: 8, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: "float" } }, // brdfLUT
                { binding: 9, visibility: GPUShaderStage.FRAGMENT, sampler: { type: "filtering" } },   // envSampler
                { binding: 10, visibility: GPUShaderStage.FRAGMENT, sampler: { type: "filtering" } },  // brdfSampler
                { binding: 11, visibility: GPUShaderStage.FRAGMENT, buffer: { type: "uniform" } },     // envParams
            ],
        });
    ...
    private updateMaterial(material: Material): void {
        ...
        const alignedSize = Math.ceil(Math.max(uniformData.byteLength, 16) / 16) * 16;
