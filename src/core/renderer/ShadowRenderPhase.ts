...
export class ShadowRenderPhase extends RenderPhase {
	...
	execute(commandEncoder: GPUCommandEncoder): void {
		if (!this.shadowsEnabled || !this.scene || this.batches.length === 0)
			return;
	...
	private renderShadowForLight(
		...
		for (const batch of this.batches) {
			...
			const renderBindGroup = this.batchManager.getRenderBindGroup(
				batch,
				cameraIndex,
			);
			...
			const indirectOffset =
				this.batchManager.getIndirectBufferOffset(cameraIndex);
