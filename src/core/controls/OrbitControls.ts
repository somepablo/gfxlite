...
export class OrbitControls {
	...
	constructor(
		...
		if (options.minPolarAngle !== undefined)
			this.minPolarAngle = options.minPolarAngle;
		if (options.maxPolarAngle !== undefined)
			this.maxPolarAngle = options.maxPolarAngle;
	...
	private onPointerMove(event: PointerEvent): void {
		...
		if (this.pointerType === "rotate") {
			// Rotate
			const rotateScale =
				((2 * Math.PI) / this.domElement.clientHeight) * this.rotateSpeed;
	...
	private onWheel(event: WheelEvent): void {
		...
		if (event.deltaY < 0) {
			this.scale /= 0.95 ** this.zoomSpeed;
	...
	update(): void {
		...
		this.spherical.phi = Math.max(
			0.0001,
			Math.min(Math.PI - 0.0001, this.spherical.phi),
		);
